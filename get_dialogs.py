...
directory = str(Path(app.name).parent)
print(directory, api_id, api_hash, phone)
...
async def create_session() -> Optional["me"]:
    ...
    result = None
    try:
        client_telethon = TelegramClient(app.name, api_id, api_hash)
        await client_telethon.start(phone=phone)
